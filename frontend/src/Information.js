...
export default function Information() {
  ...
  const handleCartClick  = () => navigate(userLocal.id ? "/cart" : "/login");
  const handleUserClick  = () => navigate(userLocal.id ? "/information" : "/login");
      ...
      <div className="relative z-10 flex flex-col min-h-screen pt-24">
        ...
        <nav className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-10 py-5 backdrop-blur-md bg-black/70 border-b border-white/10">
          ...
          <div className="flex gap-6 items-center text-gray-300">
            ...
            <button onClick={handleCartClick}><ShoppingCart className="hover:text-white transition" size={22} /></button>
            <button onClick={handleUserClick}>
              {customer?.avatar ? (
                <img src={customer.avatar} alt="avatar" className="w-8 h-8 rounded-full object-cover ring-2 ring-white/20" />
              ) : (
          ...
          <aside className="w-60 bg-black/30 border-r border-white/5 flex flex-col py-8 px-4 shrink-0">
            ...
            <div className="flex flex-col items-center mb-8">
              <div className="relative group">
                ...
                {/* Nút đổi avatar - chỉ hiện với normal */}
              ...
              {[
                ...
                <button key={key} onClick={() => setActiveTab(key)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition
                    ${activeTab === key ? "bg-white text-black font-semibold" : "text-white/50 hover:bg-white/5 hover:text-white"}`}>
                  <Icon size={16} />{label}
                </button>
              ))}
            </nav>
              ...
              <div className="max-w-2xl mx-auto flex flex-col gap-6">
                ...
                <Section title="Thông tin cá nhân">
                  ...
                  <EditableRow
                    ...
                    isEditing={editPhone}
                    inputValue={phone}
                    onInputChange={(v) => { setPhone(v); setErrors({}); }}
                  ...
                  <EditableRow
                    ...
                    inputValue={address}
                    onInputChange={(v) => { setAddress(v); setErrors({}); }}
                    ...
                    value={
                      loginTypeLabel[customer?.login_type] ? (
                        <span className={`text-xs px-2 py-1 rounded-full font-medium
                          ${customer.login_type === "google"   ? "bg-blue-500/20 text-blue-300" : ""}
                          ${customer.login_type === "facebook" ? "bg-indigo-500/20 text-indigo-300" : ""}`}>
                          {loginTypeLabel[customer?.login_type]}
                        </span>
                      ) : (
                    ...
                    <div className="flex items-start justify-between py-3 border-b border-white/5">
                      ...
                      {editPass ? (
                        <div className="flex-1 flex flex-col gap-2">
                          <PasswordInput placeholder="Mật khẩu hiện tại"   value={passForm.current}
                          ...
                          <PasswordInput placeholder="Mật khẩu mới"        value={passForm.newPass}
                          ...
                          <PasswordInput placeholder="Nhập lại mật khẩu mới" value={passForm.confirm}
                          ...
                          <div className="flex gap-2 mt-1">
                            <button onClick={savePassword} disabled={saving}
                              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white text-black text-xs font-medium hover:opacity-80 transition">
                              <Check size={12} /> Lưu
                            </button>
                        ...
                        <div className="flex-1 flex items-center justify-between">
                          ...
                          <button onClick={() => setEditPass(true)}
                            className="p-1.5 rounded-lg hover:bg-white/5 text-white/30 hover:text-white transition">
                            <Pencil size={14} />
                          </button>
                        </div>
                      )}
        ...
        {isEditing ? (
          <div className="flex-1 flex items-center gap-2">
            <input value={inputValue} onChange={(e) => onInputChange(e.target.value)}
              className={`flex-1 bg-white/5 border rounded-lg px-3 py-1.5 text-sm outline-none focus:border-white/30 transition
                ${error ? "border-red-500/50" : "border-white/10"}`} />
            <button onClick={onSave} disabled={saving}
              className="p-1.5 rounded-lg bg-white text-black hover:opacity-80 transition"><Check size={14} /></button>
            <button onClick={onCancel}
              className="p-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition"><X size={14} /></button>
          </div>
        ) : (
          <div className="flex-1 flex items-center justify-between">
            <span className="text-sm">{value || <span className="text-white/20 italic">Chưa cập nhật</span>}</span>
            <button onClick={onEdit}
              className="p-1.5 rounded-lg hover:bg-white/5 text-white/30 hover:text-white transition"><Pencil size={14} /></button>
          </div>
        )}
      </div>
    ...
    <div>
      <div className="relative">
        <input type={show ? "text" : "password"} placeholder={placeholder} value={value}
          ...
          className={`w-full bg-white/5 border rounded-lg px-3 py-1.5 pr-9 text-sm outline-none focus:border-white/30 transition
            ${error ? "border-red-500/50" : "border-white/10"}`} />
